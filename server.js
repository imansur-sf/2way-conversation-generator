...
function draftPrompt({ companyName, website, useCase, channels, evidence }) {
  ...
  return `Create concise, realistic Salesforce two-way messaging demo content. Return JSON only.\nCompany supplied: ${companyName || '(not supplied)'}\nWebsite: ${website}\nUse case: ${useCase}\nChannels: ${channels.join(', ')}\n\nWebsite evidence:\nTitle: ${evidence.title}\nDescription: ${evidence.description}\nHeadings: ${evidence.headings.slice(0,10).join(' | ')}\nText: ${evidence.text.slice(0,3500)}\nImage candidates (only use these URLs or empty strings):\n${images}\n\nReturn: {"companyName":"","initials":"","emailAddress":"","logoUrl":"","heroImageUrl":"","brandColor":"#0176D3","brandSecondaryColor":"#032D60","scenarios":{"sms":{"title":"","sender":"","initialMessage":"","keywords":[{"terms":"","response":""}],"fallbackResponse":""},"rcs":{"title":"","initialMessage":"","keywords":[{"terms":"","response":""}],"fallbackResponse":"","cards":[{"title":"","description":"","cta":"","url":"","imageUrl":""}]},"whatsapp":{"title":"","initialMessage":"","keywords":[{"terms":"","response":""}],"fallbackResponse":"","cards":[{"title":"","description":"","cta":"","url":"","imageUrl":""}]},"email":{"title":"","subject":"","preheader":"","initialBody":"","ctaLabel":"","ctaUrl":"","layout":"hero","prefilledReply":"","keywords":[{"terms":"","response":""}],"fallbackResponse":""}}}. For brandColor and brandSecondaryColor, provide 6-digit hex colors inferred from the company logo or website; use #0176D3 and #032D60 only when the evidence provides no reliable palette. For email, use the logo and hero image candidates when relevant, write a realistic preheader and CTA, and choose layout "hero" or "simple". Include only requested channel keys. Use exactly 2 keyword responses and one fallback per requested channel; include at most 2 rich cards for RCS or WhatsApp. Keep every text field under 240 characters. Do not invent URLs or facts not supported by the evidence.`;
}
...
function normalizeDraft(raw, channels, website) {
  ...
  channels.forEach(channel => {
    ...
    scenarios[channel] = { title:clean(scenario.title,`${clean(raw?.companyName,'Company')} ${channel.toUpperCase()} conversation`), sender:clean(scenario.sender,clean(raw?.companyName,hostname)), initialMessage:clean(scenario.initialMessage), subject:clean(scenario.subject,`A message from ${clean(raw?.companyName,hostname)}`), preheader:clean(scenario.preheader), initialBody:clean(scenario.initialBody), ctaLabel:clean(scenario.ctaLabel), ctaUrl:clean(scenario.ctaUrl), layout:['hero','simple'].includes(clean(scenario.layout)) ? clean(scenario.layout) : 'hero', prefilledReply:clean(scenario.prefilledReply), keywords:responses(scenario.keywords), fallbackResponse:clean(scenario.fallbackResponse,'Thanks for reaching out. I can help you find the best next step.'), cards:Array.isArray(scenario.cards) ? scenario.cards.slice(0,4).map(card => ({ title:clean(card?.title,'Learn more'), description:clean(card?.description), cta:clean(card?.cta,'Learn more'), url:clean(card?.url), imageUrl:clean(card?.imageUrl) })) : [] };
  });
  return { companyName:clean(raw?.companyName,hostname), initials:clean(raw?.initials).replace(/[^A-Za-z0-9]/g,'').slice(0,3).toUpperCase(), emailAddress:clean(raw?.emailAddress,`hello@${hostname}`), logoUrl:clean(raw?.logoUrl), heroImageUrl:clean(raw?.heroImageUrl), brandColor:color(raw?.brandColor), brandSecondaryColor:color(raw?.brandSecondaryColor,'#032D60'), scenarios };
}
